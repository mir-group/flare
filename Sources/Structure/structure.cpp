...
Structure::Structure(const Eigen::MatrixXd &cell,
  ...
  set_structure(cell, coded_species, positions, coded_mass_dict, prev_positions,
                label_input);
}
...
const Eigen::MatrixXd &Structure ::get_cell() const { return cell; }
...
const Eigen::MatrixXd &Structure ::get_positions() const { return positions; }
...
void StructureDescriptor ::nested_descriptors() {
  ...
  for (int i = 0; i < nat; i++) {
    env = LocalEnvironment(*this, i, cutoff, n_body_cutoffs, many_body_cutoffs,
                           descriptor_calculators);
    env.compute_descriptors_and_gradients();
    local_environments.push_back(env);
  }
