...
train_structure = Structure(cell, species, train_positions)
test_structure = Structure(cell, species, test_positions)
...
##    print(py_var)
#
#    struc = test_cpp_struc_pow1.descriptors[0]
#    n_descriptors = np.shape(struc.descriptors[0])[1]
#    n_species = len(atom_types)
...
##    for s in range(n_species):
##        n_struc = struc.n_clusters_by_type[s];
##        for i in range(n_struc):
##            norm = struc.descriptor_norms[s][j]
##            n_neigh = struc.neighbor_counts[s][j]
##            c_neigh = struc.cumulative_neighbor_counts[s][j]
##            atom_index = struc.atom_indices[s][j]
##            for k in range(n_neigh):
##                ind = c_neigh + k
##                neighbor_index = struc.neighbor_indices[s][ind];
##                for comp in range(3):
##                    f1 = struc.descriptor_force_dervs[s][3 * ind + comp] / norm
##                    f2 = struc.descriptors[s] * struc.descriptor_force_dots[s][3 * ind + comp] / norm ** 3
##                    f3 = f1 - f2
