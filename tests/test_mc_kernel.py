import pytest
import numpy as np
import sys
from random import random, randint
from copy import deepcopy
from flare import env, gp, struc
import flare.mc_simple as en


# -----------------------------------------------------------------------------
#                        test two plus three body kernels
# -----------------------------------------------------------------------------

def generate_envs(cutoffs, delta):
    # create env 1
    cell = np.eye(3)

    positions_1 = [np.array([0., 0., 0.]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()])]
    positions_2 = deepcopy(positions_1)
    positions_2[0][0] = delta
    positions_3 = deepcopy(positions_1)
    positions_3[0][0] = -delta

    species_1 = [1, 2, 1, 2, 1, 1, 2, 1, 3]
    atom_1 = 0
    test_structure_1 = struc.Structure(cell, species_1, positions_1)
    test_structure_2 = struc.Structure(cell, species_1, positions_2)
    test_structure_3 = struc.Structure(cell, species_1, positions_3)

    env1_1 = env.AtomicEnvironment(test_structure_1, atom_1, cutoffs)
    env1_2 = env.AtomicEnvironment(test_structure_2, atom_1, cutoffs)
    env1_3 = env.AtomicEnvironment(test_structure_3, atom_1, cutoffs)


    # create env 2
    positions_1 = [np.array([0., 0., 0.]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()]),
                   np.array([random(), random(), random()])]
    positions_2 = deepcopy(positions_1)
    positions_2[0][1] = delta
    positions_3 = deepcopy(positions_1)
    positions_3[0][1] = -delta

    atom_2 = 0
    species_2 = [1, 1, 2, 1, 2, 1, 2, 2, 2]

    test_structure_1 = struc.Structure(cell, species_2, positions_1)
    test_structure_2 = struc.Structure(cell, species_2, positions_2)
    test_structure_3 = struc.Structure(cell, species_2, positions_3)

    env2_1 = env.AtomicEnvironment(test_structure_1, atom_2, cutoffs)
    env2_2 = env.AtomicEnvironment(test_structure_2, atom_2, cutoffs)
    env2_3 = env.AtomicEnvironment(test_structure_3, atom_2, cutoffs)

    return env1_1, env1_2, env1_3, env2_1, env2_2, env2_3

# TODO: fix this test to properly account for factors of 2 and 3
def test_two_plus_three_body_force_en():
    """Check that the analytical force/en kernel matches finite difference of
    energy kernel."""

    cutoffs = np.array([1, 1])
    delta = 1e-8
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    # set hyperparameters
    sig1 = random()
    ls1 = random()
    sig2 = random()
    ls2 = random()
    d1 = 1

    hyps = np.array([sig1, ls1, sig2, ls2])

    # check force kernel
    calc1 = en.two_body_mc_en(env1_2, env2_1, hyps[0:2], cutoffs)
    calc2 = en.two_body_mc_en(env1_1, env2_1, hyps[0:2], cutoffs)
    calc3 = en.three_body_mc_en(env1_2, env2_1, hyps[2:4], cutoffs)
    calc4 = en.three_body_mc_en(env1_1, env2_1, hyps[2:4], cutoffs)

    kern_finite_diff = (calc1 - calc2) / (2 * delta) + \
        (calc3 - calc4) / (3 * delta)
    kern_analytical = \
        en.two_plus_three_mc_force_en(env1_1, env2_1, d1, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(-kern_finite_diff, kern_analytical, atol=tol))


def test_two_plus_three_body_force():
    """Check that the analytical force kernel matches finite difference of
    energy kernel."""

    # create env 1
    delta = 1e-5
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    # set hyperparameters
    sig1 = random()
    ls1 = random()
    sig2 = random()
    ls2 = random()
    d1 = 1
    d2 = 2

    hyps = np.array([sig1, ls1, sig2, ls2])

    # check force kernel
    calc1 = en.two_plus_three_mc_en(env1_2, env2_2, hyps, cutoffs)
    calc2 = en.two_plus_three_mc_en(env1_3, env2_3, hyps, cutoffs)
    calc3 = en.two_plus_three_mc_en(env1_2, env2_3, hyps, cutoffs)
    calc4 = en.two_plus_three_mc_en(env1_3, env2_2, hyps, cutoffs)

    kern_finite_diff = (calc1 + calc2 - calc3 - calc4) / (4*delta**2)
    kern_analytical = en.two_plus_three_body_mc(env1_1, env2_1,
                                             d1, d2, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(kern_finite_diff, kern_analytical, atol=tol))


def test_two_plus_three_body_grad():
    # create env 1

    cutoffs = np.array([1, 1])
    delta = 1e-8
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    # set hyperparameters
    sig1 = random()
    ls1 = random()
    sig2 = random()
    ls2 = random()

    d1 = randint(1, 3)
    d2 = randint(1, 3)


    hyps = np.array([sig1, ls1, sig2, ls2])
    hyps1 = np.array([sig1+delta, ls1, sig2, ls2])
    hyps2 = np.array([sig1, ls1+delta, sig2, ls2])
    hyps3 = np.array([sig1, ls1, sig2+delta, ls2])
    hyps4 = np.array([sig1, ls1, sig2, ls2+delta])

    grad_test = en.two_plus_three_body_mc_grad(env1_1, env2_1, d1, d2, hyps, cutoffs)

    sig1_derv_brute = (en.two_plus_three_body_mc(env1_1, env2_1, d1, d2,
                                              hyps1, cutoffs) -
                       en.two_plus_three_body_mc(env1_1, env2_1, d1, d2,
                                              hyps, cutoffs)) / delta

    l1_derv_brute = \
        (en.two_plus_three_body_mc(env1_1, env2_1, d1, d2, hyps2, cutoffs) -
         en.two_plus_three_body_mc(env1_1, env2_1, d1, d2, hyps, cutoffs)) / delta

    sig2_derv_brute = \
        (en.two_plus_three_body_mc(env1_1, env2_1, d1, d2,
                                hyps3, cutoffs) -
         en.two_plus_three_body_mc(env1_1, env2_1, d1, d2,
                                hyps, cutoffs)) / delta

    l2_derv_brute = \
        (en.two_plus_three_body_mc(env1_1, env2_1, d1, d2, hyps4, cutoffs) -
         en.two_plus_three_body_mc(env1_1, env2_1, d1, d2, hyps, cutoffs)) / delta

    tol = 1e-4
    assert(np.isclose(grad_test[1][0], sig1_derv_brute, atol=tol))
    assert(np.isclose(grad_test[1][1], l1_derv_brute, atol=tol))
    assert(np.isclose(grad_test[1][2], sig2_derv_brute, atol=tol))
    assert(np.isclose(grad_test[1][3], l2_derv_brute, atol=tol))

# -----------------------------------------------------------------------------
#                              test two body kernels
# -----------------------------------------------------------------------------

def test_two_body_force_en():
    """Check that the analytical force/en kernel matches finite difference of
    energy kernel."""

    # create env 1
    delta = 1e-8
    cell = np.eye(3)
    cutoffs = np.array([1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = random()
    ls = random()
    d1 = 1

    hyps = np.array([sig, ls])

    # check force kernel
    calc1 = en.two_body_mc_en(env1_2, env2_1, hyps, cutoffs)
    calc2 = en.two_body_mc_en(env1_1, env2_1, hyps, cutoffs)

    kern_finite_diff = (calc1 - calc2) / delta
    kern_analytical = en.two_body_mc_force_en(env1_1, env2_1, d1, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(-kern_finite_diff/2, kern_analytical, atol=tol))

def test_two_body_force():
    """Check that the analytical force kernel matches finite difference of
    energy kernel."""

    # create env 1
    delta = 1e-5
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = 1
    ls = 0.1
    d1 = 1
    d2 = 2

    hyps = np.array([sig, ls])

    # check force kernel
    calc1 = en.two_body_mc_en(env1_2, env2_2, hyps, cutoffs)
    calc2 = en.two_body_mc_en(env1_3, env2_3, hyps, cutoffs)
    calc3 = en.two_body_mc_en(env1_2, env2_3, hyps, cutoffs)
    calc4 = en.two_body_mc_en(env1_3, env2_2, hyps, cutoffs)

    kern_finite_diff = (calc1 + calc2 - calc3 - calc4) / (4*delta**2)
    kern_analytical = en.two_body_mc(env1_1, env2_1,
                                  d1, d2, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(kern_finite_diff, kern_analytical, atol=tol))


def test_two_body_grad():

    delta = 1e-8
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = random()
    ls = random()
    d1 = randint(1, 3)
    d2 = randint(1, 3)

    hyps = np.array([sig, ls])

    grad_test = en.two_body_mc_grad(env1_1, env2_1, d1, d2, hyps, cutoffs)

    new_sig = sig + delta
    new_ls = ls + delta

    sig_derv_brute = (en.two_body_mc(env1_1, env2_1, d1, d2,
                                  np.array([new_sig, ls]),
                                  cutoffs) -
                      en.two_body_mc(env1_1, env2_1, d1, d2,
                                  hyps, cutoffs)) / delta

    l_derv_brute = (en.two_body_mc(env1_1, env2_1, d1, d2,
                                np.array([sig, new_ls]),
                                cutoffs) -
                    en.two_body_mc(env1_1, env2_1, d1, d2,
                                hyps, cutoffs)) / delta

    tol = 1e-4
    assert(np.isclose(grad_test[1][0], sig_derv_brute, atol=tol))
    assert(np.isclose(grad_test[1][1], l_derv_brute, atol=tol))


# -----------------------------------------------------------------------------
#                              test three body kernels
# -----------------------------------------------------------------------------


def test_three_body_force_en():
    """Check that the analytical force/en kernel matches finite difference of
    energy kernel."""

    # create env 1
    delta = 1e-8
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = random()
    ls = random()
    d1 = 1

    hyps = np.array([sig, ls])

    # check force kernel
    calc1 = en.three_body_mc_en(env1_2, env2_1, hyps, cutoffs)
    calc2 = en.three_body_mc_en(env1_1, env2_1, hyps, cutoffs)

    kern_finite_diff = (calc1 - calc2) / delta
    kern_analytical = en.three_body_mc_force_en(env1_1, env2_1, d1, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(-kern_finite_diff/3, kern_analytical, atol=tol))


def test_three_body_force():
    """Check that the analytical force kernel matches finite difference of
    energy kernel."""

    # create env 1
    delta = 1e-5
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = 1
    ls = 0.1
    d1 = 1
    d2 = 2

    hyps = np.array([sig, ls])

    # check force kernel
    calc1 = en.three_body_mc_en(env1_2, env2_2, hyps, cutoffs)
    calc2 = en.three_body_mc_en(env1_3, env2_3, hyps, cutoffs)
    calc3 = en.three_body_mc_en(env1_2, env2_3, hyps, cutoffs)
    calc4 = en.three_body_mc_en(env1_3, env2_2, hyps, cutoffs)

    kern_finite_diff = (calc1 + calc2 - calc3 - calc4) / (4*delta**2)
    kern_analytical = en.three_body_mc(env1_1, env2_1,
                                    d1, d2, hyps, cutoffs)

    tol = 1e-4
    assert(np.isclose(kern_finite_diff, kern_analytical, atol=tol))


def test_three_body_grad():
    # create env 1
    delta = 1e-8
    cutoffs = np.array([1, 1])
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(cutoffs, delta)

    sig = random()
    ls = random()
    d1 = randint(1, 3)
    d2 = randint(1, 3)

    hyps = np.array([sig, ls])

    grad_test = en.three_body_mc_grad(env1_1, env2_1, d1, d2, hyps, cutoffs)

    new_sig = sig + delta
    new_ls = ls + delta

    sig_derv_brute = (en.three_body_mc(env1_1, env2_1, d1, d2,
                                    np.array([new_sig, ls]),
                                    cutoffs) -
                      en.three_body_mc(env1_1, env2_1, d1, d2,
                                    hyps, cutoffs)) / delta

    l_derv_brute = (en.three_body_mc(env1_1, env2_1, d1, d2,
                                  np.array([sig, new_ls]),
                                  cutoffs) -
                    en.three_body_mc(env1_1, env2_1, d1, d2,
                                  hyps, cutoffs)) / delta

    tol = 1e-4
    assert(np.isclose(grad_test[1][0], sig_derv_brute, atol=tol))
    assert(np.isclose(grad_test[1][1], l_derv_brute, atol=tol))
