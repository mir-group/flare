...
def test_start(parameter, bodies, same_hyps, prefix):
    ...
    for comb in list_of_triplet:
        for species in  set(permutations(comb)):
            ...
            reference = get_reference(grid_env, species, parameter, kernel_name, same_hyps)

            coords = np.zeros((1, grid_dim * 3))
            coords[:, 0] = np.ones_like(coords[:, 0])

            fj, fdj = xb_map.bonds_cutoff(grid, hm['cutoffs'][kernel_name],
                                     coords, derivative=True, cutoff_func=quadratic_cutoff)
            fdj = fdj[:, [0]]

            hyps, r_cut = xb_map.get_hyps_for_kern(hm['hyps'], hm['cutoffs'], 
                    hyps_mask, grid_env.ctype, grid_env.etypes)

            kern_type = f'{prefix}_force'
            kern_vec = kernel(env, bodies, kern_type, xb_map.get_bonds_for_kern, xb_map.bonds_cutoff, 
