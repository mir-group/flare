...
def test_vasp_input_edit():
    ...
    assert np.isclose(
        final_structure.positions[0], structure.positions[0], atol=1e-4
    ).all()
...
def test_run_dft_par():
    ...
    for dft_command in [None]:
        with raises(FileNotFoundError):
            run_dft_par("POSCAR", test_structure, dft_command=dft_command, n_cpus=2)
...
def test_md_trajectory():
    ...
    structures = md_trajectory_from_vasprun(
        "test_files/test_vasprun.xml", ionic_step_skips=2
    )
