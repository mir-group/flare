...
def test_force_en_multi_vs_simple(kernels, multi_cutoff):
    ...
    print(kernels, i, reference, result)
    assert(isclose(reference[0], result[0], rtol=tol).all())
    joint_grad = np.zeros([len(result[1])//2, 3, 3])
...
def test_check_sig_scale(kernels, diff_cutoff):
    ...
    * if the separate group of hyperparameter is properly
    ...
    for idx in range(reference[1].shape[0]):
        # check sig0
        if np.min(reference[1][idx]) and (idx % 4 == 1):
            assert isclose(result[1][idx]/reference[1][idx], scale*np.ones(result[1][idx].shape), rtol=tol).all()
        # check the rest, but skip sig 1
        elif np.min(reference[1][idx]) and (idx % 4 == 3):
