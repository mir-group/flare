...
from .fake_gp import generate_hm, generate_envs, generate_mb_envs

list_to_test = ['2', '3', 'mb', '2+3', '2+3+mb']

# @pytest.mark.parametrize('kernel_name', list_to_test)
# @pytest.mark.parametrize('diff_cut', [True, False])
# def test_force_en_multi_vs_simple(kernel_name, diff_cut):
...
#     tol = 1e-4
#
#     cutoffs, hyps0, hyps1, hyps2, hm1, hm2 = generate_same_hm(kernel_name, diff_cut)
#
#     # mc_simple
...
@pytest.mark.parametrize('kernel_name', list_to_test)
@pytest.mark.parametrize('diff_cut', [True, False])
def test_force_bound_cutoff_compare(kernel_name, diff_cut):
    """Check that the analytical kernel matches the one implemented
    in mc_simple.py"""
    ...
    cutoffs, hyps, hm = generate_diff_hm(kernel_name, diff_cut, False)
...
# @pytest.mark.parametrize('kernel_name', list_to_test)
# @pytest.mark.parametrize('diff_cut', [True, False])
# def test_force_en(kernel_name, diff_cut):
#     """Check that the analytical force/en kernel matches finite difference of
#     energy kernel."""
#
#     delta = 1e-5
...
def test_force(kernel_name, constraint):
    ...
    cutoffs, hyps, hm = generate_diff_hm(kernel_name, constraint)
    env1_1, env1_2, env1_3, env2_1, env2_2, env2_3 = generate_envs(
        cutoffs, delta, hm)
...
def generate_same_hm(kernel_name, diff_cut=False):
    cutoffs = []
    hyps0 = []
    hyps2 = []
...
def generate_diff_hm(kernel_name, diff_cut=False, constraint=False):
    cutoffs = []
    hyps2 = []
    hm2 = {'nspec': 2, 'spec_mask': np.zeros(118, dtype=int)}
    hm2['spec_mask'][2] = 1
...
def test_many_force():
    ...
    cutoffs, hyps, hm = generate_diff_hm('mb')
    args = from_mask_to_args(hyps, hm, cutoffs)
