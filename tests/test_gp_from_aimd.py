...
# TO DO: this need to be rewrite for the per_species
def test_uncertainty_threshold(fake_gp):
    ...
    res1, res2 = is_std_in_bound_per_species(tt.rel_std_tolerance,
            tt.abs_std_tolerance, noise, fake_structure, 2,
            std_per_species = False)
    assert res1 is True
    assert res2 == [-1]
    ...
    os.system('rm gp_from_aimd.out*')
    os.system('rm gp_from_aimd-f.xyz*')
    os.system('rm gp_from_aimd-hyps.dat*')
