...
void SparseGP::write_mapping_coefficients(std::string file_name,
  ...
  // Format time in buffer yyyy-mm-dd-Thh:mm:ss
  strftime (buffer,80,"%Y-%m-%d-T%H:%M:%S",timeinfo);

  // Record Metadata (date, contributor)
  jsonfile["metadata"]["date"] = buffer;
  jsonfile["metadata"]["contributor"] =  contributor;

  // Record kernel information (name, power)
  kernels[kernel_index]->write_info(jsonfile);

  // Record descriptor information 
  ...
  for (int i = 0; i < mapping_coeffs.rows(); i++) {
    ...
    for (int j = 0; j < coeff_vals.size(); j++) {
      ...
      jsonfile["mapping"]["coefficients"].push_back(coeff_vals[j]);
      count++;
    }
  }
