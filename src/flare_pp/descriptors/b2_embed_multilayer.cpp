...
B2_Embed_Multilayer ::B2_Embed_Multilayer() {}

B2_Embed_Multilayer ::B2_Embed_Multilayer(const std::string &radial_basis, const std::string &cutoff_function,
  ...
  // Species embedding coefficients
  embed_coeffs = embed_coefficients;
}
...
void B2_Embed_Multilayer ::write_to_file(std::ofstream &coeff_file, int coeff_size) {
...
DescriptorValues B2_Embed_Multilayer ::compute_single_bonds(Structure &structure) {
  DescriptorValues desc = DescriptorValues();
  return desc;
}

DescriptorValues B2_Embed_Multilayer ::compute_struc(Structure &structure) {
  ...
  single_bond_multiple_cutoffs_embed(
  ...
  compute_b2_embed_multilayer(B2_vals, B2_force_dervs, B2_norms, B2_force_dots, single_bond_vals,
             force_dervs, unique_neighbor_count, cumulative_neighbor_count,
             descriptor_indices, embed_coeffs.rows(), N, lmax);
...
nlohmann::json B2_Embed_Multilayer ::return_json(){
  nlohmann::json j;
  return j;
}
