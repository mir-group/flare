...
B2_Embed ::B2_Embed(const std::string &radial_basis, const std::string &cutoff_function,
...
void B2_Embed ::write_to_file(std::ofstream &coeff_file, int coeff_size) {
...
DescriptorValues B2_Embed ::compute_struc(Structure &structure) {
  ...
  compute_b2_embed(B2_vals, B2_force_dervs, B2_norms, B2_force_dots, single_bond_vals,
...
#pragma omp parallel for
  ...
  int single_bond_size = no_bond_vals * embed_coeffs.rows(); //nos;
      ...
      for (int embed_counter = 0; embed_counter < embed_coeffs.rows(); embed_counter++) {
        ...
        for (int radial_counter = 0; radial_counter < N; radial_counter++) {
          ...
          for (int angular_counter = 0; angular_counter < number_of_harmonics;
            ...
            force_dervs(neighbor_index * 3 + 1, descriptor_counter) += bond_y * embed_coeffs(embed_counter, ind);
            force_dervs(neighbor_index * 3 + 2, descriptor_counter) += bond_z * embed_coeffs(embed_counter, ind);
