...
void CompactStructures ::add_structure(const CompactStructure &structure){
  ...
  if (n_strucs == 0){
      ...
      for (int i = 0; i < n_species; i++){
          ...
          descriptors[i].conservativeResize(
            e_rows + e_rows_struc, n_descriptors);
          descriptor_force_dervs[i].conservativeResize(
            f_rows + f_rows_struc, n_descriptors);
          descriptor_stress_dervs[i].conservativeResize(
              s_rows + s_rows_struc, n_descriptors);

          // Update.
          descriptors[i].block(e_rows, 0, e_rows_struc, n_descriptors) =
            structure.descriptors[i];
          descriptor_force_dervs[i].block(
              f_rows, 0, f_rows_struc, n_descriptors) =
              structure.descriptor_force_dervs[i];
          descriptor_stress_dervs[i].block(
              s_rows, 0, s_rows_struc, n_descriptors) =
              structure.descriptor_stress_dervs[i];
      }
  }
  ...
  if (n_strucs == 0) c_atoms.push_back(0);
  else c_atoms.push_back(c_atoms[n_strucs-1] + n_atoms[n_strucs-1]);
