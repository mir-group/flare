...
void CompactEnvironments ::add_environments(
    const CompactStructure &structure, std::vector<int> environments) {
  ...
  for (int s = 0; s < n_species; s++){
      descriptors[s].conservativeResize(
          n_atoms[s] + species_count[s], n_descriptors);
  }
  ...
  for (int i = 0; i < environments.size(); i++){
    ...
    descriptor_norms[s].push_back(
        structure.descriptor_norms[s](species_index));
    species_count(s) ++;
  }
