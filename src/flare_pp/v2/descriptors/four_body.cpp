...
DescriptorValues FourBody ::compute_struc(CompactStructure &structure){
  ...
  desc.n_types =
    n_species * (n_species + 1) * (n_species + 2) * (n_species + 3) /
    (2 * 3 * 4);
    ...
    for (int j = 0; j < i_neighbors; j++) {
      ...
      for (int k = 0; k < i_neighbors; k++){
        ...
        for (int l = 0; l < i_neighbors; l++){
          ...
          if (l_species < k_species) continue;
    ...
    for (int j = 0; j < i_neighbors; j++) {
      ...
      for (int k = 0; k < i_neighbors; k++){
        ...
        for (int l = 0; l < i_neighbors; l++){
          ...
          desc.cutoff_values[current_type](count) =
          ...
          for (int k = 0; k < 3; k++){
            ...
            // Second neighbor.
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 3 + k, 1) = neigh_coord_2 / r2;
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 3 + k, 2) = coord_diff_1 / r3;
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 3 + k, 5) = -coord_diff_3 / r6;
            ...
            desc.descriptor_force_dots[current_type](count * 3 * 3 + 3 + k) =
              desc.descriptor_force_dervs[current_type]
              .row(count * 3 * 3 + 3 + k).dot(
              desc.descriptors[current_type].row(count));

            // Third neighbor.
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 6 + k, 3) = neigh_coord_3 / r4;
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 6 + k, 4) = coord_diff_2 / r5;
            desc.descriptor_force_dervs[current_type]
              (count * 3 * 3 + 6 + k, 5) = coord_diff_3 / r6;
