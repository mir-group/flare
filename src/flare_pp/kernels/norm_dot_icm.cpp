...
#pragma omp parallel for
      for (int i = 0; i < n_sparse; i++) {
        ...
        for (int j = 0; j < n_struc; j++) {
          ...
          kern_mat(sparse_index, 0) +=
            sig_sq * icm_val * pow(norm_dot, power);
          sig_mat(sparse_index, 0) +=
            2 * sig_new * icm_val * pow(norm_dot, power);
          icm_mats[icm_index](sparse_index, 0) +=
            sig_sq * pow(norm_dot, power); 
          ...
          for (int k = 0; k < n_neigh; k++) {
            ...
            for (int comp = 0; comp < 3; comp++) {
              ...
              icm_mats[icm_index](sparse_index, 1+3*neighbor_index+comp) -=
                  icm_force_derv;
              icm_mats[icm_index](sparse_index, 1+3*neighbor_index+comp) -=
                  icm_force_derv;
...
NormalizedDotProduct_ICM ::struc_struc(const DescriptorValues &struc1,
  ...
  //         struc1.descriptor_force_dervs[s] *
  //         struc2.descriptors[s].transpose();
  //     Eigen::MatrixXd force_dot_2 =
  //         struc2.descriptor_force_dervs[s] *
  //         struc1.descriptors[s].transpose();
  ...
  //             // Energy/force.
  //             kernel_matrix(0, 1 + 3 * neighbor_index + comp) -=
  //             force_kern_val; kernel_matrix(0, 1 + 3 * c_ind_2 + comp) +=
  //             force_kern_val;
  ...
  //             // Force/energy.
  //             kernel_matrix(1 + 3 * neighbor_index + comp, 0) -=
  //             force_kern_val; kernel_matrix(1 + 3 * c_ind_1 + comp, 0) +=
  //             force_kern_val;
  ...
  //         // Force/force, force/stress, stress/force, and stress/stress
  //         kernels. for (int k = 0; k < n_neigh_1; k++) {
  ...
  //                             norm_dot * struc_force_dot_1(f_ind_1) /
  //                             norm_i2;
  //                 double v2 = force_dot_2(f_ind_2, i) / norm_ij -
  //                             norm_dot * struc_force_dot_2(f_ind_2) /
  //                             norm_j2;
  ...
  //                   for (int q = n; q < 3; q++) {
  //                     double coord2 = struc2.neighbor_coordinates[s](ind2,
  //                     q); kernel_matrix(1 + 3 * struc1.n_atoms +
  //                     stress_ind_1,
  //                                   1 + 3 * struc2.n_atoms + stress_ind_2) +=
...
int get_icm_index(int s1, int s2, int n_types){
  ...
  int icm_index =
    (n_types * (n_types + 1) / 2) - (s_diff * (s_diff + 1) / 2);
  return icm_index;
}
