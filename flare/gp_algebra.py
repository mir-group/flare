...
def get_ky_and_hyp_pack(
    ...
    assert size1 == size2 and s1 == 0 and s2 == 0 and same
    ...
    # distribute work of lower-triangular matrix
    N = size1 * (size1 + 1) // 2
    ...
    for x in range(local_starts[rank], local_ends[rank]):
        ...
        m_index = math.floor((-1 + math.sqrt(1 + 8 * x)) / 2)
        n_index = x - m_index * (m_index + 1) // 2
        ...
        # store kernel value
        k_mat_triang[x] = cov[0]
        grad = from_grad_to_mask(cov[1], hyps_mask)
        hyp_mat_triang[x, :] = grad

    comm.Allgatherv(MPI.IN_PLACE, [k_mat_triang, local_sizes, local_starts, MPI.DOUBLE])
    comm.Allgatherv(
        MPI.IN_PLACE,
        [
            hyp_mat_triang,
            non_noise_hyps * local_sizes,
            non_noise_hyps * local_starts,
            MPI.DOUBLE,
        ],
    )
    # comm.Allreduce(MPI.IN_PLACE, k_mat_triang)
    # comm.Allreduce(MPI.IN_PLACE, hyp_mat_triang)

    # copy flattened lower triangular to "proper" matrices
    for i in range(size1):
        for j in range(i + 1):
            x = i * (i + 1) // 2 + j
            k_mat[i, j] = k_mat_triang[x]
            k_mat[j, i] = k_mat_triang[x]
            hyp_mat[:, i, j] = hyp_mat_triang[x, :]
            hyp_mat[:, j, i] = hyp_mat_triang[x, :]
...
def get_ky_and_hyp(
    ...
    assert n_cpus == 1, "ERROR: Use MPI and n_cpus=1"
